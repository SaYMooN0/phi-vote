...
trait AccessTokenReader {
  def read(token: String): IO[AccessTokenReadErr, AccessTokenClaims]
}
final case class AccessTokenClaims(userId: AppUserId, tokenId: String, expiresAt: Instant)

enum AccessTokenReadErr {
  case Malformed
  case InvalidSignature
  case Expired
  case InvalidIssuerOrAudience
  case InvalidSubject
  case Other(exMsg: String)
}
...
object RsaPublicKeyPem {
  def parse(publicKeyPem: String): Task[RSAPublicKey] =
    ZIO.attempt {
      val normalized =
        publicKeyPem
          .replace("\\n", "\n")
...
private final class AccessTokenReaderLive private(
  ...
  override def read(token: String): IO[AccessTokenReadErr, AccessTokenClaims] =
    for {
      decoded <- ZIO
  ...
  private def parseUserId(subject: String): IO[AccessTokenReadErr.InvalidSubject.type, AppUserId] =
    ZIO
      .attempt(AppUserId(UUID.fromString(subject)))
      .mapError(_ => AccessTokenReadErr.InvalidSubject)

  private def parseTokenId(decoded: DecodedJWT): IO[AccessTokenReadErr.Malformed.type, String] =
    Option(decoded.getId) match {
      case Some(value) if value.nonEmpty => ZIO.succeed(value)
      case _ => ZIO.fail(AccessTokenReadErr.Malformed)
    }

  private def parseExpiresAt(decoded: DecodedJWT): IO[AccessTokenReadErr.Malformed.type, Instant] =
    Option(decoded.getExpiresAt) match {
      case Some(value) => ZIO.succeed(value.toInstant)
      case None => ZIO.fail(AccessTokenReadErr.Malformed)
    }

  private def toReadErr(err: Throwable): AccessTokenReadErr =
    err match {
      case _: TokenExpiredException => AccessTokenReadErr.Expired
      case _: SignatureVerificationException => AccessTokenReadErr.InvalidSignature
      case _: AlgorithmMismatchException => AccessTokenReadErr.InvalidSignature
      case _: JWTDecodeException => AccessTokenReadErr.Malformed
      case _: InvalidClaimException => AccessTokenReadErr.InvalidIssuerOrAudience
      case e: JWTVerificationException => AccessTokenReadErr.Other(e.getMessage)
      case e => AccessTokenReadErr.Other(e.getMessage)
    }
}
  ...
  val layer: ZLayer[AccessTokenReaderConfig, Throwable, AccessTokenReader] =
    ZLayer {
      for {
        ...
        publicKey <- RsaPublicKeyPem.parse(config.publicKeyPem)
