...
import backend.authservice.domain.services.RefreshTokenSecurityService
...
private final class RefreshTokenSecurityServiceLive private(
  ...
  override def generate: Task[RefreshTokenPlain] = {
    ZIO.attempt {
  ...
  override def hash(token: RefreshTokenPlain): Task[RefreshTokenHash] = {
    ZIO.attempt {
      ...
      val secretKey = SecretKeySpec(config.hmacSecret.getBytes("UTF-8"), "HmacSHA256")
      mac.init(secretKey)
      val hash = Base64
        .getUrlEncoder
        .withoutPadding()
        .encodeToString(mac.doFinal(token.value.getBytes("UTF-8")))
